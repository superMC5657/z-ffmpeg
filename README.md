...
- **FFmpeg 自动下载**：未检测到 FFmpeg 时可从镜像（国内加速优先）自动下载安装；日志与队列数据统一存放在 `{data_dir}/z-ffmpeg/`。
...
推送 `v*` tag 触发 CI：类型检查、eslint、vitest、cargo test 全部通过后，构建 NSIS 安装包并发布 GitHub Release（含 updater 签名产物与国内镜像 `latest-cn.json`）。版本号用 `pnpm bump:version` 三处同步（package.json / Cargo.toml / tauri.conf.json）。
